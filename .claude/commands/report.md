...
If no argument, report on all projects in `projects/_registry.json`.

Steps:
1. Read `projects/_registry.json`
2. For each project in scope, read `projects/{repo-name}.md`
...
4. Compute or verify the current score using the formula in `projects/README.md`
5. Update `projects/{repo-name}.md` with a new score card row and session note
6. Update `last_updated` in `projects/_registry.json`
